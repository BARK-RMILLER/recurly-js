...
const debug = require('debug')('recurly:adyen:strategy');
/**

 * KakaoPay base interface strategy
 *
 * @abstract
 */
export class KakaoPayStrategy extends Emitter {
  constructor (options) {
    super();
    this.isReady = false;
    this.config = {};
  ...
  configure (options) {
    if (!(options.recurly instanceof Recurly)) throw this.error('kakaopay-factory-only');
